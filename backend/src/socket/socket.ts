...
import { deleteFilesFromCloudinary, uploadAudioToCloudinary } from "../utils/auth.util.js";
    ...
    io.on("connection",async(socket:Socket)=>{
        ...
        socket.on(Events.MESSAGE,async({chatId,isPollMessage,pollData,textMessageContent,url,encryptedAudio,audio,replyToMessageId}:MessageEventReceivePayload)=>{
            ...
            if(audio){
                ...
                const uploadResult = await uploadAudioToCloudinary({buffer:encryptedAudio, folder: 'encrypted-audio'});
                if(!uploadResult) return;
