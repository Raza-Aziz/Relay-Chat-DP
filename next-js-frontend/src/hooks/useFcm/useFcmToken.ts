...
import { isSupported, onMessage, Unsubscribe } from "firebase/messaging";
...
const useFcmToken = () => {
  ...
  const loadToken = useCallback(async () => {
    ...
    try {
      ...
      if (!fcmSupported) {
        console.info(
          "FCM is not supported in this browser; push token will be skipped."
        );
        return;
      }
      ...
      if (!process.env.NEXT_PUBLIC_FIREBASE_VAPID_KEY) {
        console.info(
          "NEXT_PUBLIC_FIREBASE_VAPID_KEY is not set; push notifications are disabled."
        );
        return;
      }

      let nextToken = await getNotificationPermissionAndToken();
      ...
      // Only retry when permission is granted but the token is missing (e.g. service worker not ready).
      if (!nextToken && Notification.permission === "granted") {
        const maxAttempts = 3;
        for (let attempt = 1; attempt <= maxAttempts && !nextToken; attempt++) {
          await new Promise((r) => setTimeout(r, 400 * attempt));
          nextToken = await fetchToken();
        }
      }
      ...
      if (!nextToken) {
        if (Notification.permission === "granted") {
          ...
          console.info(
            "%cPush Notifications issue - unable to load FCM token after retries",
            "color: green; background: #c7c7c7; padding: 8px; font-size: 20px"
          );
        }
        return;
